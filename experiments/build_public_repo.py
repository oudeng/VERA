...
def main() -> int:
    ...
    #: Only a build into the CANONICAL location may write the release record.
    #: A --target build is a dry run -- diffing the published tree against
    #: what today's code produces, say -- and it must not silently retarget
    #: the letters and the manuscript at a commit that was never pushed.
    #: It did exactly that once, and the letters' --check caught it.
    if stage.resolve() != (PROJ / "VERA_GitHub").resolve():
        print(f"[release] not recorded: --target build at {stage}, not the "
              f"canonical tree; results/public_release.json left untouched")
        return 0
