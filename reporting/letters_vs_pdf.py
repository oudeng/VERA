...
def pointers(text: str) -> list[str]:
    """Every path-shaped literal in a letter, FILE and DIRECTORY alike.

    The first version matched only `dir/name.ext`, which is half the class:
    the letters also send a reader to `prereg_archive/`, `experiments/` and
    `sni/` as directories, and a directory that does not exist in the
    published tree is exactly as unfollowable as a missing file. All six
    happened to resolve; that they were never checked is the defect.
    """
    files = re.findall(r"`([\w./-]+/[\w./-]+\.\w+)`", text)
    dirs = re.findall(r"`([\w][\w./-]*/)`", text)
    return sorted(set(files) | set(dirs))
...
def reachable() -> dict:
    ...
    for name in LETTERS:
        ...
        for rel in pointers(f.read_text()):
            ...
            elif not (PUBLIC / rel.rstrip("/")).exists():
...
def run() -> dict:
    ...
    #: Two documents, two page numberings. "Sect. 12 (p. 25)" is a valid
    #: pointer into the Online Resource and a wrong one into the manuscript,
    #: so the namespaces are kept apart rather than pooled.
    printed = {(v[0], v[1]) for k, v in loc.items() if not k.startswith("esm:")}
    printed_esm = {(v[0], v[1]) for k, v in loc.items() if k.startswith("esm:")}
    ...
    for name in LETTERS:
        ...
        for m in re.finditer(
                r"(Online Resource 1, )?(?:Sect\.|Table|Fig\.) ([\d.]+) "
                r"\(p\. (\d+)\)", t):
            n_ptr += 1
            into_esm = bool(m.group(1))
            want = printed_esm if into_esm else printed
            doc = "Online Resource" if into_esm else "manuscript"
            if (m.group(2), m.group(3)) not in want:
                bad.append(f"{name}: '{m.group(0)}' names no label the "
                           f"{doc} defines at that number and page")
...
def _selftest() -> int:
    ...
    #: directories are checked too, not only files
    c(any(p.endswith("/") for p in pointers(
        (ROOT / "VERA_response" / LETTERS[0]).read_text())),
      "directory-style pointers are in scope, not only files")
    ...
    #: the two documents' pointers must not be pooled
    _l = locations()
    _m = {(v[0], v[1]) for k, v in _l.items() if not k.startswith("esm:")}
    _e = {(v[0], v[1]) for k, v in _l.items() if k.startswith("esm:")}
    c(_e - _m, "the Online Resource defines locations the manuscript does not, "
               "so pooling them would accept wrong pointers")
